...
object VariableRefRewriter extends Rewriter {
  ...
  private val rewriter = Rewriter.lift {
    case p: LogicalPlan => p match {
        ...
        case p @ BFSPruningVarExpand(_, from, _, _, maybeTo, _, _, depthName, _, _, _, _) =>
          p.copy(from = varRef(from), maybeTo = maybeTo.map(varRef), depthName = depthName.map(varRef))(SameId(p.id))
        ...
        case p @ PruningVarExpand(_, from, _, _, maybeTo, _, _, _, _, _) =>
          p.copy(from = varRef(from), maybeTo = maybeTo.map(varRef))(SameId(p.id))
        ...
        case p @ TransactionApply(_, _, _, _, _, maybeReportAs, _) =>
          p.copy(maybeReportAs = maybeReportAs.map(varRef))(SameId(p.id))
        case p @ TransactionForeach(_, _, _, _, _, maybeReportAs, _) =>
          p.copy(maybeReportAs = maybeReportAs.map(varRef))(SameId(p.id))
        ...
        case p @ PartitionedUnwindCollection(_, variable, _) =>
          p.copy(variable = varRef(variable))(SameId(p.id))
        case p @ VarExpand(_, from, _, _, _, maybeTo, maybeRel, _, _, _, _, _) =>
          p.copy(from = varRef(from), maybeTo = maybeTo.map(varRef), maybeRelName = maybeRel.map(varRef))(SameId(p.id))
        case other => other
      }
