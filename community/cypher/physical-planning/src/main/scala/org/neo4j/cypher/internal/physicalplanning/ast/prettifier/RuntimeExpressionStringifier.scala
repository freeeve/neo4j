...
object RuntimeExpressionStringifier extends ExpressionStringifier.Extension {

  override def apply(ctx: ExpressionStringifier)(expression: Expression): String = expression match {
    case e: SlottedCachedProperty => removeGeneratedNamesAndParams(e.propertyAccessString)
    case e: ParameterFromSlot     => s"$$${ctx.backtick(e.name)}"
    case _: NodeProperty | _: NodePropertyLate | _: RelationshipProperty | _: RelationshipPropertyLate =>
      removeGeneratedNamesAndParams(expression.asCanonicalStringVal)
    case _: NodePropertyExists | _: NodePropertyExistsLate | _: RelationshipPropertyExists | _: RelationshipPropertyExistsLate =>
      s"${removeGeneratedNamesAndParams(expression.asCanonicalStringVal)} IS NOT NULL"
