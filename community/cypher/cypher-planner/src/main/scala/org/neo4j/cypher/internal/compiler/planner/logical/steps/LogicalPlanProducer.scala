...
case class LogicalPlanProducer(
  ...
  def planVarExpand(
    ...
    patternRelationship.length match {
      ...
      case _ =>
        throw InternalException.internalError(this.getClass.getSimpleName, "Expected a varlength path to be here")
    }
  }
  ...
  def planRepeat(
    ...
    try {
      ...
      // But if it does, we should not need to forward the error to the user.
      case _: InternalException => throw InternalException.internalError(
