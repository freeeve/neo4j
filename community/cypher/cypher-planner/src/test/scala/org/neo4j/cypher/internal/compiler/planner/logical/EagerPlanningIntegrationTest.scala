  ...
  test("eagerness should handle matching on dynamic labels - Create overlap – using dynamic label scan") {
    ...
    plan should equal(
      planner.planBuilder()
        ...
        .apply()
        .|.dynamicLabelNodeLookup("n", varFor("labels"), DynamicElement.Any, IndexOrderNone, "labels")
        .projection("['A', 'B'] AS labels")
        .argument()
        .build()
    )
  ...
  test("eagerness should handle matching on Dynamic Labels - Delete overlap – using dynamic label scan") {
    ...
    plan should equal(
      planner.planBuilder()
        ...
        .apply()
        .|.cartesianProduct()
        .|.|.dynamicLabelNodeLookup("n", listOf(), DynamicElement.All, IndexOrderNone, "types")
        .|.filter("NOT m:%")
        .|.allNodeScan("m", "types")
  ...
  test("insert eager between MATCH with dynamic relationship type and CREATE – using dynamic label scan") {
    ...
    plan shouldEqual
      planner.planBuilder()
        ...
        .apply()
        .|.dynamicRelationshipTypeLookup("(a)-[r]->(b)", "$any(types)", IndexOrderNone, "types")
        .projection("['A', 'B'] AS types")
        .argument()
        .build()
  }
  ...
  test("insert eager between MATCH with dynamic relationship type and DELETE – using dynamic label scan") {
    ...
    plan should equal(
      planner.planBuilder()
        ...
        .apply()
        .|.cartesianProduct()
        .|.|.dynamicRelationshipTypeLookup("()-[r]-()", "$any(types)", IndexOrderNone, "types")
        .|.filter("NOT s:A")
        .|.allRelationshipsScan("()-[s]->()", "types")
        .projection("['A', 'B'] AS types")
        .argument()
