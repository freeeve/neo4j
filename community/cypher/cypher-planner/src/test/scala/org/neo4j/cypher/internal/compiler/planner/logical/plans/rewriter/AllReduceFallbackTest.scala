...
class AllReduceFallbackTest extends CypherFunSuite with AstConstructionTestSupport {
  ...
  test("allReduce translation to reduce") {
    // allReduce(acc = 0, acc + n.x, acc < 11)
    val allReducePred = allReduce(
      accumulator = v"acc",
      init = literalInt(0),
      groupVariable = v"n",
      allReduceStepExpression = add(v"acc", prop(v"n", "x")),
      allReducePredicate = lessThan(v"acc", literalInt(11))
    )

    QueryRenderer.pretty(allReducePred) shouldBe "allReduce(acc = 0, acc + n.x, acc < 11)"
    ...
    QueryRenderer.pretty(allReduceFallback) shouldBe
      """reduce(acc = {accumulator: 0, result: true}, n IN n | CASE
        |  WHEN acc.result = false THEN acc
        |  ELSE [`  UNNAMED0` IN [acc.accumulator + n.x] | {accumulator: `  UNNAMED0`, result: acc.result AND `  UNNAMED0` < 11}][0]
  ...
  test("allReduce translation to reduce - conjunction in allReducePredicate") {
    // allReduce(acc = 1, acc * n.x + 1, 1 < acc < 11 AND acc <> 5)
    val allReducePred = allReduce(
      accumulator = v"acc",
      init = literalInt(1),
      groupVariable = v"n",
      allReduceStepExpression = add(multiply(v"acc", prop(v"n", "x")), literalInt(1)),
      allReducePredicate =
        ands(lessThan(literalInt(1), v"acc"), lessThan(v"acc", literalInt(11)), notEquals(v"acc", literalInt(5)))
    )

    QueryRenderer.pretty(allReducePred) shouldBe "allReduce(acc = 1, acc * n.x + 1, 1 < acc AND acc < 11 AND acc <> 5)"
    ...
    QueryRenderer.pretty(allReduceFallback) shouldBe
      """reduce(acc = {accumulator: 1, result: true}, n IN n | CASE
        |  WHEN acc.result = false THEN acc
        |  ELSE [`  UNNAMED0` IN [acc.accumulator * n.x + 1] | {accumulator: `  UNNAMED0`, result: acc.result AND (1 < `  UNNAMED0` AND `  UNNAMED0` < 11 AND `  UNNAMED0` <> 5)}][0]
        |END).result""".stripMargin
