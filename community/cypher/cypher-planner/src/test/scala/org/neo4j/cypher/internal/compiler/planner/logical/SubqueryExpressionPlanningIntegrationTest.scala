...
class SubqueryExpressionPlanningIntegrationTest extends CypherFunSuite with LogicalPlanningIntegrationTestSupport
  ...
  test("should solve pattern comprehension for LetSelectOrAntiSemiApply") {
    ...
    plan should equal(
      planner.subPlanBuilder()
        .selectOrAntiSemiApply("anon_6")
        .|.filter("anon_4:O")
        .|.expandAll("(n)-[anon_3:Q]->(anon_4)")
        .|.argument("n")
        .letSelectOrAntiSemiApply("anon_6", "n.prop = reduce(sum = 0, x IN anon_5 | sum + x)")
        .|.filter("anon_2:M")
        .|.expandAll("(n)-[anon_1:R]->(anon_2)")
        .|.argument("n")
  ...
  test("should solve pattern comprehension for LetSelectOrSemiApply") {
    ...
    plan should equal(
      planner.subPlanBuilder()
        .selectOrSemiApply("anon_6")
        .|.filter("anon_4:O")
        .|.expandAll("(n)-[anon_3:Q]->(anon_4)")
        .|.argument("n")
        .letSelectOrSemiApply("anon_6", "n.prop = reduce(sum = 0, x IN anon_5 | sum + x)")
        .|.filter("anon_2:M")
        .|.expandAll("(n)-[anon_1:R]->(anon_2)")
        .|.argument("n")
        .rollUpApply("anon_5", "anon_0")
