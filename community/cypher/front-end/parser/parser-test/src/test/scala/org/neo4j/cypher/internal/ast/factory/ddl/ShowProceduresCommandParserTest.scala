    ...
    test(s"USE db SHOW $procKeyword") {
      assertAstVersionBased(cypher5 =>
        SingleQuery(
          List(
            use(List("db"), !cypher5),
            ShowProceduresClause(None, None, List.empty, yieldAll = false, None, hasOrderByOnYield = false)((1, 8, 7))
          )
        )((1, 8, 7))
      )
    }
