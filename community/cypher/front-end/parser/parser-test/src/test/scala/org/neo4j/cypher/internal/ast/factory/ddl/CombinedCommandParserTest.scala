...
class CombinedCommandParserTest extends AdministrationAndSchemaCommandParserTestBase {
  ...
  test("MATCH (n) TERMINATE TRANSACTION") {
    parsesIn[ast.Statements] {
      ...
      case _ => _.withSyntaxError(
          """Invalid input 'TERMINATE': expected a graph pattern, ',', 'ORDER BY', 'CALL', 'CREATE', 'LOAD CSV', 'DELETE', 'DETACH', 'FILTER', 'FINISH', 'FOREACH', 'INSERT', 'LET', 'LIMIT', 'MATCH', 'MERGE', 'NEXT', 'NODETACH', 'OFFSET', 'OPTIONAL', 'REMOVE', 'RETURN', 'SEARCH', 'SET', 'SKIP', 'UNION', 'UNWIND', 'USE', 'USING', 'WHERE', 'WITH' or <EOF> (line 1, column 11 (offset: 10))
            |"MATCH (n) TERMINATE TRANSACTION"
            |           ^""".stripMargin
        )
    }
