...
/**
 * This annotation marks a GQLSTATUS code as having a non-sensitive status description.
 * This should be used when the status description cannot contain any sensitive or user data and is thus safe to log
 * un-obfuscated to the query log.
 * The annotation does not have any effect for GQLSTATUS codes for non-errors, i.e. with classifications NO_DATA,
 * WARNING, SUCCESSFUL_COMPLETION and INFORMATIONAL, as those are not logged.
 */
@Target(ElementType.FIELD)
@Retention(RetentionPolicy.RUNTIME)
public @interface NonSensitiveStatusDescription {
    Reason[] reasons();

    /*
     * General rule: a status description is considered sensitive if it has any parameters which might contain
     ...
     */
    enum Reason {
        // Status descriptions without parameters are just static text so cannot contain sensitive data
        NO_PARAMETERS,
